# Script de test final pour le mapping userId -> accountRef
# Utilise les ports corrects
...
$testUsername = Read-Host "Entrez le nom d'utilisateur Keycloak"
$testPassword = Read-Host "Entrez le mot de passe" -AsSecureString
...
try {
    ...
    if ($_.Exception.Response) {
        ...
        if ($statusCode -eq 404) {
            Write-Host "  L'utilisateur n'a pas de compte. Creez-en un d'abord." -ForegroundColor Yellow
    ...
    Write-Host "  3. Pour creer un compte: POST $accountServiceUrl/api/accounts" -ForegroundColor Yellow
